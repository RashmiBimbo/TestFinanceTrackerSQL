...
BEGIN
    ...
    BEGIN TRY
        ...
        INSERT INTO @tbl
        (            
            [USER_ID] ,[REPORT_ID] ,[Report_Type] ,[Add_Date] ,[MONTH_FROM_DATE] ,[MONTH_TO_DATE] ,[MONTH_WEEK_NO], [Year_Half_No] ,[LOCATION] ,[CREATED_BY]
		)
        SELECT *
        FROM
            OPENJSON(@Collection)
        ...
        WHEN NOT MATCHED THEN                 
	        ...
	        VALUES
	        (
	            UPPER(TRIM([Source].[USER_ID])) ,[Source].[REPORT_ID] ,UPPER(TRIM([Report_Type])) ,[Source].[Add_Date] ,[Source].[MONTH_FROM_DATE] ,[Source].[MONTH_TO_DATE] , [Source].[MONTH_WEEK_NO], [Source].[Year_Half_No], [Source].[LOCATION] ,[Source].[CREATED_BY]
	        );
